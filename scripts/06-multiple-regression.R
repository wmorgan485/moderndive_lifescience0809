...
variances_interaction <- get_regression_points(len_model_interaction) %>% 
  summarize(var_y = var(len), var_y_hat = var(len_hat), var_residual = var(residual)) %>% 
  mutate(model = "Interaction", r_squared = var_y_hat/var_y)
variances_parallel_slopes <- get_regression_points(len_model_parallel_slopes) %>% 
  summarize(var_y = var(len), var_y_hat = var(len_hat), var_residual = var(residual)) %>% 
  mutate(model = "Parallel slopes", r_squared = var_y_hat/var_y)
